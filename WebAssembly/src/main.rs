...
use std::{collections::HashMap, convert::TryInto, fs::File, io::Read, path::PathBuf};
use structopt::StructOpt;
use tracing::{debug_span, trace};
use walrus::{
    ir::{InstrSeq, InstrSeqId, InstrSeqType},
...
struct Context<'a, 'wasm> {
    ...
    memory_limits: HashMap<RegisterKind, u32>,
...
impl<'a, 'wasm> Context<'a, 'wasm> {
    fn new(
        ...
        assert_eq!(initial, 0);
        // Reserve an instruction slot for reqbl
        this.body.blocks[0].stmts.push(Instruction::Nop.into());
...
fn transpile_wasm_function_body_to_scale<'a, 'wasm>(
    cx: &'a Compiler,
    module: &'wasm walrus::Module,
    func: &'wasm walrus::LocalFunction,
    exports: HashMap<String, &'wasm ExportItem>,
    ...
    if let Some(reqbl) = context.reqbl {
        context.body.blocks[0].stmts[0] = Instruction::General {
            destinations: vec![],
            instruction: "reqbl",
