...
# FIXME: set `-Cinline-threshold=99999`
env SHARING_DATA=$ROOT/Data/SharingData.txt RUSTC_BOOTSTRAP=1 RUSTFLAGS="-Ctarget-feature=+simd128,+unimplemented-simd128 -C inline-threshold=99999 --remap-path-prefix $ROOT=" cargo build --target wasm32-unknown-unknown --manifest-path=$ROOT/RustPrograms/Cargo.toml --release --example $test || exit 1
...
OPTS="${OPTS=--always-inline-max-function-size=99999 --flexible-inline-max-function-size=99999 --one-caller-inline-max-function-size=99999}"
