...
As a further caveat the conversion is only possible in certain situations. 
In particular we must have $\log_2 p > 64$, otherwise we would loose precision in the conversion. 
There are however additional {\em security} considerations which need to be
satisfied.
...
We first take $\log_2 p$ daBits $r_i$ and then forming the integer $r = \sum r_i \cdot 2^i$
...
For this latter method to be secure we need that the value $r$ generated above is 
either large enough to drown out a $64$-bit value, or is itself sufficiently close 
to uniformly random to be secure.
Thus we require one of three conditions on $p$ to be met:
\begin{enumerate}
...
\end{lstlisting}
\begin{footnotesize}
Note: x type can be: sregint, sbit, sint, sfix, and sfloat.
\end{footnotesize}
...
In the case of clear data types, they work as expected for integers of bit length $n$. Whereas for \verb|sbit|, they operate as in boolean algebra. Note that the $\&$ operand is also supported between \verb|sregint| and \verb|sbit| types 
