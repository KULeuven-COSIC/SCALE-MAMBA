...
\func{cfix(value)}
It instantiates  an \verb|sfix| register based on \verb|value|. Note that \verb|value| is a publicly available value, it loads it as a secret fix point. 
\\
\textbf{Example:}
\begin{lstlisting}
i = cfix(5.5)
...
We now provide some basic examples of basic operations. Both, integer and fractional data-types, can behave in different ways. To better exemplify their use, we utilize \verb|sint| and \verb|sfix|, but these examples can be extended for any other integer and fractional type i.e., (\verb|cint| and \verb|cfix|).
...
%You can think of $f$ as the bitwise precision for the given fixed point representation.
	...
	S	Stores an integer on the $\{-2^k-1, 2^k-1\}$ interval, encoding of the rational original value.
	\func{f}
	\textbf{Accessed by:} \verb|Default|.\\
	\textbf{Type:} \verb|int|.\\
...
\func{sfix.sizeof()}
			...
			\textbf{Description:}
				It returns the global_vector_size times 4. \\
	     	\textbf{Example:}
...
of a statistical security parameter $\kappa$ for \verb|cfix| values.
	...
	\textbf{Accessed by:} \verb|Default|.\\
	\textbf{Type:} \verb|int|.\\
	Stores an integer on the $\{-2^k-1, 2^k-1\}$ interval, encoding of the rational original value.
	\func{f}
	\textbf{Accessed by:} \verb|Default|.\\
	\textbf{Type:} \verb|int|.\\
	Stores the bitwise bit precision of the value to be stored by the instance.
...
\func{cfix.set_precision(f, k = None)}
    		...
    		\textbf{Returns:} No return value. \\
     		\textbf{Description:}
				Let you change the precision for \verb|cfix|. 
                                The requirement is that $f<k$ and $2 \cdot k + \kappa < \log_2 p$.
                                By default the values $(f,k)=(20,41)$ are chosen internally in the compiler.
                                It is used to fix the default precision on \verb|types.py|. \\
			\textbf{Example:}
...
There is also a statistical security parameter $\kappa$ associated with an \verb+sfloat+ which needs
to satisfy
