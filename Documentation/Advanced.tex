...
\msubsubsection{$\mathsf{NormSQ}(\sshare{b},k,f):$}
...
\begin{enumerate}
\item $\sshare{x_{k-1}},\ldots,\sshare{x_0} \asn \mathsf{BitDec}(\sshare{b},k,k)$.
\item $\sshare{y_{k-1}},\ldots,\sshare{y_0} \asn \mathsf{PreOp}(\mathsf{OR},\sshare{x_{k-1}},\ldots,\sshare{x_0},k)$.
\item For $i \in [0,\ldots,k-2]$ do 
\begin{enumerate}
  \item $\sshare{z_i} \asn \sshare{y_i}-\sshare{y_{i+1}}$.
...
\item For $i \in [1,\ldots,k/2]$ do
\begin{enumerate}
	\item $\sshare{w_i}=\sshare{z_{2\cdot i-1 -(1 -k\%2)}}+\sshare{z_{2\cdot i - (1-k\%2)}}$.
\end{enumerate}
...
The fixed point square root algorithm is in
\begin{itemize}
...
\msubsubsection{$\mathsf{FxSqrt}(\sshare{x},k,f)$:}
