...
\item $\sshare{w} \asn sfix( \sshare{w} * 2 ^{\frac{f - f \quad \% \quad 2}{2}})$.
...
We can now provide a more simplified mechanism to call our \verb|FxSqrt| functionality. Note that our \verb|SimplifiedFxSqrt| requires some extra rounds, given the need of performing a division. We use it only when  the $3 \cdot k - 2 \cdot f < \mathtt{sfix.f}$ bound is not met. 
