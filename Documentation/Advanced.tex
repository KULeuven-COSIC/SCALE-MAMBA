...
\msubsubsection{$\mathsf{Mod2m}(\sshare{a_prime}, \sshare{a},k,m, \kappa, signed)$:}
...
\msubsubsection{$\mathsf{Mod2m}(a_prime, \sshare{a},k,\sshare{m}, \kappa)$:}
...
\todo{This section is currently not fully implemented. We aim to 
implement it fully in the next release.}
...
a computation.
\todo{This error flag is not currently implemented in MAMBA system 
we need to add it}
...
\msubsubsection{$\mathsf{FlowDetect}(\sshare{p},\sshare{\err})$:}
\begin{enumerate}
...
\todo{As floating point is not yet fully implemented, the
references in this section to floating point operations should
...
\item Return $\sshare{g}$.
\end{enumerate}
\paragraph{MAMBA Example:} To cast an \verb|sfloor| register into a \verb|sint| one, you could execute the following: 
\begin{lstlisting}[language={python}]
from Compiler import floatingpoint
...
We have also included a more practical SQRT construction, so that it could be used under global precision and bit size parameters.
...
This algorithm uses the sub-algorithm $\mathsf{NormSQ}$ defined above. In our case we make use of $w$, which is basically  $2^{m/2}$. From that point it approximates the  value of $\sqrt{x}$ by calculating $\frac{x}{2^{m/2}}$. To avoid any loss of precision, we reuse \verb|sfix| instantiation process.
...
\msubsubsection{$\mathsf{FxSqrt}(\sshare{x}, k \asn sfix.k, f \asn sfix.f)$:}
...
\todo{These functions are only currently supported in their
...
\todo{In the current version, all trigonometric functionality is implemented for fixed-point types only.
Extension to floating point values will come soon.}
