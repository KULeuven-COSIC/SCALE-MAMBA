...
In addition each online thread is supported by four other threads
performing the offline phase, each again with pairwise 
TLS secured point-to-point channels.

In the case of Full Threshold secret sharing another thread
acts as a factory of FHE ciphertexts. Actively secure
production of such ciphertexts is expensive, requiring complex
zero-knowledge proofs (see Section \ref{sec:fhe}). Thus
the FHE-Factory thread locates this production into a
single location. This saves memory, over a multiple factory
design.
...
\begin{itemize}
\item \verb|ci|: clear $\F_p$ register index $i$
\item \verb|si|: secret $\F_p$ register index $i$
\item \verb|v|: immediate value in $\F_p$
\item \verb|n|: integer value
\end{itemize}
...
To enable debugging we provide simple commands to print 
values to standard output. These bytecodes are
\begin{verbatim}
...
\begin{verbatim}
             ...
             START_PRIVATE_INPUT,    STOP_PRIVATE_INPUT, 
             START_PRIVATE_OUTPUT,   STOP_PRIVATE_OUTPUT. 
\end{verbatim}
...
\verb+start_clock(n)+ and \verb+stop_clock(n)+.
The timers use the basic \verb+clock()+ command in C, and so have the
same limitations as that function.
\end{itemize}
