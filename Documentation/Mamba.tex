...
\end{lstlisting}
The functions are called by pushing data onto the requesite stacks,
executing a \verb+CALL+ bytecode, then the arguments are popped off,
and the function is run.
Return arguments are done using pushing, calling the \verb+RETURN+ 
bytecode, and then popping the arguments off.
