...
if test "$1"; then
    ...
    for test in test_array test_array_arith test_bitdecfull test_GC test_bitprotocols test_ieee test_modp test_sregint test_local_functions test_stacks test_sfix test_sfix_lib test_oram test_sfloat test_sfloat_lib test_constants; do
	run_test $test
    done
    printf "\n\n\n\n\n\n\n\n\n\n"
fi
