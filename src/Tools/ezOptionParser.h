...
public:
  ...
  inline ezOptionValidator(char _type, char _op, const long long *list,
                           int _size);
  inline ezOptionValidator(char _type, char _op, const unsigned long long *list,
                           int _size= 0);
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator(char _type, char _op,
                                     const unsigned long long *list, int _size)
              ...
              if (dvalue < dmin)
                {
                  if (!quiet)
                    {
                      fprintf(stderr,
                              "ERROR: Invalid value %g is less than datatype min %g.\n",
                              dvalue, dmin);
                    }
                  return false;
                }
              ...
              for (int j= 0; j < n; ++j)
                {
                  ...
                  for (int k= 0; k < m; ++k)
                    {
                      quote= ((*g->args[j]->at(k)).find_first_of(" \t") !=
                              std::string::npos);
                      if (quote)
                        out.append("\"");
              ...
              if (sq.size())
                {
                  ...
                  // All start of strings will be even indices, closing quotes is odd
                  // indices.
                  j= (int) (lo - sq.begin());
                  if ((j % 2) == 0)
                                          ...
                                          Layout layout)
{
  ...
  // Number of whitespaces from start of line to description (interleave layout)
  // or
  ...
  for (i= 0; i < (long int) groups.size(); ++i)
    {
      ...
      // (offset, length) tuples, but just to get it done, we'll do some not-too
      // expensive string copying.
              ...
              while ((long int) rem->size() > helpwidth)
                {
                  ...
                  if (rem->at(helpwidth) == ' ')
                    ...
                    {
                      // Otherwise, split occurs midword, so find whitespace before this
                      // word.
